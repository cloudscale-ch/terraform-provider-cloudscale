		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						resourceName, "network_name", fmt.Sprintf("terraform-%d", rInt)),
					resource.TestCheckResourceAttrSet(
						resourceName, "network_href"),
					resource.TestCheckResourceAttr(
						resourceName, "type", "private"),
					resource.TestCheckResourceAttrSet(
						resourceName, "mac_address"),
					resource.TestCheckResourceAttr(
						resourceName, "addresses.#", "1"),
					resource.TestCheckResourceAttr(
						resourceName, "addresses.0.address", "10.11.12.10"),
					resource.TestCheckResourceAttrPair(
						resourceName, "addresses.0.subnet_uuid", "cloudscale_subnet.basic", "id"),
					resource.TestCheckResourceAttr(
						resourceName, "addresses.0.subnet_cidr", "10.11.12.0/24"),
					resource.TestCheckResourceAttr(
						resourceName, "addresses.0.version", "4"),
					resource.TestCheckResourceAttrSet(
						resourceName, "addresses.0.subnet_href"),
				),
			},
			{
